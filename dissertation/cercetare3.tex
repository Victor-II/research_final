...
\begin{enumerate}
    \item Measure the impact of output format (structured vs.\ natural-language templates) on out-of-domain generalisation, with multi-seed statistical validation.
...
The move toward joint formulations began with the observation that aspects, opinions, and sentiments are interdependent, and that solving them together avoids error propagation. Peng et al.~\cite{peng2020knowing} introduced the Aspect Sentiment Triplet Extraction (ASTE) task, which requires extracting (aspect, opinion, polarity) triplets from text. Their approach used a two-stage pipeline: the first stage jointly extracts aspect terms with their polarities and opinion terms using separate tagging schemes, while the second stage couples the extracted aspects and opinions into valid triplets. This pipeline architecture suffers from a well-known issue: errors in span extraction propagate to the pairing stage and cannot be recovered. Subsequent work moved toward end-to-end approaches to avoid this.

Among end-to-end discriminative methods, Span-ASTE~\cite{xu2021learning} enumerated candidate spans for aspects and opinions, used a dual-channel pruning strategy to filter them, and then classified sentiment relations for each span pair. It achieved approximately 72--73 F1 on the Rest14 benchmark with a BERT encoder. Wu et al.~\cite{wu2020grid} proposed the Grid Tagging Scheme (GTS), encoding sentence structure as a 2D table where cells indicate aspect spans, opinion spans, and their sentiment relations. More recently, BTF-CCL~\cite{btfccl2025} combined table-filling with boundary-driven detection and cross-granularity contrastive learning, reaching 75.88 F1 on Rest14, the highest reported discriminative result. These methods require specialised architectures (GCN layers, table structures, contrastive objectives) to push performance beyond 73 F1. Importantly, none of them report out-of-domain evaluation results in their publications.
...
We formulate Aspect Sentiment Triplet Extraction as a sequence-to-sequence generation problem, following GAS~\cite{zhang2021towards} and the Paraphrase framework~\cite{zhang2021aspect}. Given an input sentence $x$, the model generates a text sequence $y$ encoding all sentiment triplets $(a_i, o_i, p_i)$ present in the sentence, where $a_i$ is an aspect term, $o_i$ is an opinion term, and $p_i \in \{\text{positive}, \text{negative}, \text{neutral}\}$ is the sentiment polarity.
...
\begin{enumerate}
    \item \textbf{SemEval ASTE}: Train on Rest14, test on Rest14 (in-domain), Rest15 and Rest16 (near-OOD, same domain family), and Laptop14 (out-of-domain).
...
DMASTE sentences are 3--4$\times$ longer than SemEval (55--64 tokens vs 15--17), contain 2$\times$ more triplets per sentence (3.3--4.0 vs 1.5--2.0), and have much higher implicit aspect rates (38--47\% vs 0\% in SemEval). These compounding differences make DMASTE a substantially harder cross-domain target.
...
\begin{table}[H]
\centering
\caption{Inline syntax modes (20 epochs, structured format, single seed). Included for completeness.}
...
Inline syntax modes hurt performance consistently (--2.5 to --4.5 F1 both ID and OOD). Mixing annotations into the input tokens disrupts the model's ability to identify actual sentence content. The compact mode (separate \texttt{Syntax:} line) avoids this interference but does not provide a statistically significant OOD benefit in the multi-seed evaluation.
...
\begin{table}[H]
\centering
\caption{MvP and STAR replication results (single seed, Rest14-only training).}
\label{tab:mvp-star}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
...
MvP with voting gives a small in-domain improvement (+1.3 points) but OOD performance (0.530) falls within the NL baseline's confidence interval (0.525$\pm$0.012). The voting mechanism helps compositional accuracy in-domain but does not address vocabulary or domain shift.
...
\begin{table}[H]
\centering
\caption{Curriculum learning results (28 epochs, NL format, Rest14 training, single seed).}
...
\begin{table}[H]
\centering
\caption{Aspect masking results (NL format, 28 epochs, Rest14 training, single seed). Mask fraction = 20\%.}
\label{tab:masking}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
...
\begin{table}[H]
\centering
\caption{Domain mixing results (Rest14 + DMASTE domain fraction, single seed). The NL baseline's 5-seed confidence interval for Laptop14 is 0.525$\pm$0.012.}
\label{tab:domainmix}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
...
\begin{table}[H]
\centering
\caption{Decoding strategies (NL format with compact dependency annotations, single seed).}
\label{tab:decoding}
\small
\begin{tabular}{@{}lcc@{}}
\toprule
...
\caption{F1 improvement over the structured baseline for each training strategy on Rest14 (in-domain).}
...
\caption{F1 improvement over the structured baseline for each training strategy on Laptop14 (out-of-domain).}
\label{fig:r3-ood-tornado}
\end{figure}

Figures~\ref{fig:r3-id-tornado} and~\ref{fig:r3-ood-tornado} summarise the impact of all tested strategies relative to the structured baseline. On in-domain data, most NL-based strategies provide 2--4 F1 points of improvement. On out-of-domain data, the NL baseline provides the largest gain (+8.6), while strategies like STAR and masking provide smaller improvements over structured but do not add value beyond the NL format itself.
...
Span-ASTE (paper) & \textbf{40.36} & \textbf{45.36} & \textbf{41.04} & \textbf{47.23} & \textbf{43.50} \\
\midrule
...
\midrule
Span-ASTE (paper) & \textbf{41.83} & 46.07 & 43.62 & \textbf{50.16} & 45.42 \\
\bottomrule
\end{tabular}
\end{table}
