...
None of the techniques mentioned above were designed with out-of-domain (OOD) generalisation in mind, at least not as the main goal. 
...
In this work, rather than proposing a new architecture, we take an empirical approach by test existing training strategies in a cross-domain setting, and measuring each method's contribution.
...
\begin{enumerate}
    ...
    \item Evaluate training-time strategies such as multi-ordering training, sub-task decomposition, curriculum learning, aspect masking and cross-domain data mixing for their effect on OOD performance.
...
Our formulation of ASTE is largely based on GAS~\cite{zhang2021towards} 
and the Paraphrase framework~\cite{zhang2021aspect}. Presented with an input sentence $x$, the model is expected to produce 
a text sequence $y$ encoding all triplets $(a_i, o_i, p_i)$ present in the sentence, where 
$a_i$ is an aspect term, $o_i$ is an opinion term, and $p_i \in \{\text{positive}, \text{negative}, 
\text{neutral}\}$ is the sentiment polarity. The format used to encode the triplets is discussed in the following section.
...
to separate them, so for a sentence that has the pairs [``food'', ``delicious''] and
 [``staff'', ``horrible''] the expected output would be 
 ``food is described as delicious; staff is described as horrible''. 
 Exempt from this rule are templates of singular tasks: 
 given a sentence with multiple aspects such as ``food'', ``staff'' and ``ambiance'', the expected output is
 ``the aspects discussed are food, staff and ambiance''.

The intuition for this output format is its resemblance to spoken language, which aligns much better to the tasks the model was alredy pre-trained for.


\section{Input Representation}\label{sec:syntax}

We investigate whether the addition of syntactic annotations, such as dependency tree or POS, to the input prompt leads to increased performance in cross-domain evaluation. 
Since aspect-opinion relationships are often encoded in the syntactic 
structure: ``delicious pizza'' and ``responsive touchscreen'', we hope this additional 
...
This method appends a lighter version of the SpaCy-generated dependency tree to the input prompt, focused only on nouns, verbs, adjectives and adverbs. 
We don't use the full dependency tree for two reasons: to avoid increasing the maximum input length and to avoid overwhelming the model with information that's irrelevant to the task.
...
This method is identical to previous one, except that instead of the dependency tree we use POS tags.
...
``Task:'' line. Moreover, instead of a structured output format, we use natural language templates for 
all 6 possible permutations of ASTE --- add ref to annex and upload all permutations. 
...
Following STAR~\cite{xie2025star}, we train on decomposed sub-tasks alongside the full triplet task.
 For each training example, additional instances are generated for all pairwise combinations 
 (aspect+opinion, aspect+polarity, opinion+polarity) as well as individual tasks, yielding 7$\times$ data multiplication. 
 The hypothesis is that element-level patterns learned from sub-tasks transfer better across domains.

 - we did not generate more samples, we used replace: true;
