...
\newcommand{\AbstractEN}{This report presents an empirical study of training strategies for out-of-domain generalisation in generative Aspect-Based Sentiment Analysis. We decompose Aspect Sentiment Triplet Extraction into three base objectives (aspect extraction, opinion extraction, polarity inference) and formulate the task as conditional text generation using FLAN-T5-base, where the model receives a task prefix specifying which objectives to extract and produces the results in either a structured or natural-language format. We evaluate the effect of output format, syntactic input enrichment, multi-ordering training, mixed-objective training, curriculum learning, aspect masking, cross-domain data mixing, and decoding strategies on cross-domain performance, testing on the SemEval ASTE benchmark (restaurant to laptop transfer) and the DMASTE multi-domain benchmark with multi-seed statistical validation. Our main finding is that natural-language output templates provide the largest improvement for OOD generalisation, with +8.6 F1 over structured output across 5 seeds and non-overlapping confidence intervals, while the other strategies we tested do not produce statistically significant improvements beyond the NL baseline. On the DMASTE benchmark, our model matches the best previously reported cross-domain result without any domain adaptation techniques.}
...
This chapter presents the various methods we experimented with. The performance of each is discussed in Chapter~\ref{ch:results}. Experimental Results.
...
\textcolor{red}{[TO DO] - introducere capitol}
...
A visual comparison of the triplet F1 score achieved by the best three models presented in 
Table~\ref{tab:dmaste} plus the structured output baseline is presented in Figure~\ref{fig:r3-dmaste}.
